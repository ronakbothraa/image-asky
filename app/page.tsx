...
export default function Home() {
  ...
  return (
    <>
      <main className="container mx-auto py-10 pb-48">
        ...
        <div className="h2"></div> {/* This div seems empty, consider removing or giving purpose */}
      </main>

      {/* Fixed "Ask a question" component */}
      <div className="fixed inset-x-0 bottom-0 z-50 bg-white dark:bg-gray-900 border-t border-gray-200 dark:border-gray-700 shadow-lg">
        <div className="mx-auto max-w-2xl px-4 py-4">
          <div className="space-y-4">
            <div className="flex justify-between items-center"> {/* Flex container for title and dropdown */}
              <h2 className="text-xl font-semibold">
                Ask a question about your files
              </h2>
              <div className="relative"> {/* Container for dropdown button and menu */}
                <Button
                  variant="outline"
                ...
                >
                  {selectedModel}
                  <ChevronDown className={`h-3 w-3 sm:h-4 sm:w-4 ml-1.5 sm:ml-2 transition-transform duration-200 ${isDropdownOpen ? "rotate-180" : ""}`} />
                </Button>
                {isDropdownOpen && (
                  <div className="absolute right-0 bottom-full mb-2 w-48 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-md shadow-lg z-20 py-1">
                    {aiModels.map((model) => (
                      ...
                      >
                        <span>{model}</span>
                        {selectedModel === model && <Check className="h-3 w-3 sm:h-4 sm:w-4 text-blue-500" />}
                      </button>
                    ))}
                  </div>
                )}
            ...
            <div className="relative w-full">
              <Textarea
                ...
                rows={1}
                className="w-full resize-none appearance-none block p-3 pr-36 sm:pr-40 border border-gray-300 dark:border-gray-600 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 dark:focus:ring-offset-gray-800 dark:focus:ring-blue-500 dark:bg-gray-800 dark:text-white placeholder-gray-400 dark:placeholder-gray-500"
              />
